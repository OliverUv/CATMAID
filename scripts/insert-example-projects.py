...
for u in users:
    ...
    if u['name'] == 'gerhard':
        for p in ('Default Project', 'Focussed Ion Beam (FIB)'):
            c.execute("INSERT INTO project_stack (project_id, user_id) VALUES (%s, %s)", (projects[p], u['id']))
...
helper_script = os.path.join(sys.argv[0], 'setup-tracing-for-project.py')
